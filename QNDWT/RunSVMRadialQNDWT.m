...
%permutation importance for all individual predictors; block permutation importance for groups of predictors.
%confusion tables use:columns = TRUE status and rows= PREDICTED status
% both counts and true-class percentages are saved; percentages are conditional on the true class, so each true-status
%      column sums to 100 percent.
%
% Status coding:
% 0 = Normal
% 1 = Benign
...
fprintf('Program version: PERM_IMPORTANCE_GROUPS_ESCAPEDLABELS_RESULTSSVM_2026_05_12\n');
...
for rr = 1:nRepeats %repeated nested cross-validation
    ...
    for ff = 1:outerK
        ...
        % do after fitting and tuning, on the outer test fold only
        if nPermutations > 0
...
% =========================================================================
% TeX-safe axis label helper
% =========================================================================

function out = escape_underscore_label_cellstr_local(labels)
% Escapes underscores for MATLAB TeX tick labels.
%
% Example:
%   Phase_mean becomes Phase\_mean, so MATLAB prints the underscore
%   literally instead of treating "mean" as a subscript.
%
% This fixes PNG labels such as Phase_mean, Orientation_D_all, and Level_5_all.
...
function set_random_seed_local(seed) % Avoids direct use of rng
...
keep = false(numel(groups),1); %removes accidental empty groups.
