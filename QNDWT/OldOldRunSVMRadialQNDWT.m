function Results = RunSVMRadialQNDWT(summaryFile, outDir, nRepeats, outerK, innerK, nPermutations)
%radial SVM classification for the quaternion NDWT summary.csv file
% Status coding:
%0 = Normal
%1 = Benign
%2 = InSitu
%3 = Invasive
%
if nargin < 1 || isempty(summaryFile) %defailt
    summaryFile = fullfile(pwd, 'summary.csv');
    if ~exist(summaryFile, 'file')
        summaryFile = 'C:\Brani2\WorkTAMU\Quaternion\QNDWT\summary.csv';
    end
end
...
fprintf('\nRadial SVM classification for quaternion NDWT features\n');
fprintf('Program version: PERM_IMPORTANCE_GROUPS_RESULTSSVM_2026_05_12\n');
fprintf('Summary file: %s\n', summaryFile);
fprintf('Output folder : %s\n', outDir);
fprintf('Outer CV: %d repeats of %d-fold CV\n', nRepeats, outerK);
fprintf('Inner CV: %d-fold CV for grid tuning\n', innerK);
...
%permutation importance accumulators.
% Importance = performance before permutation - performance after permutation
...
for rr = 1:nRepeats
    ...
    for ff = 1:outerK
        ...
        % Held-out permutation importance; done after fitting and tuning, on the outer test fold only
        if nPermutations > 0
        ...
        % A group is permuted as a block: one permutation of test images is applied simultaneously to all features in that group. This keeps 
        % the internal structure of the group intact, while disconnecting the whole group from the correct image labels
...
try
    ...
    xlabel('True status')
    ylabel('Predicted status')
    title(sprintf('Radial SVM confusion table, repeated %d x %d-fold CV', nRepeats, outerK));
...
try
    ...
    set(gca, 'YTick', 1:topN, ...
        'YTickLabel', flipud(importanceTable.feature(1:topN)));
    xlabel('Mean decrease in held-out balanced accuracy')
    title('Top radial-SVM permutation importance predictors')
    grid on
...
try
    ...
    set(gca, 'YTick', 1:topGroupN, ...
        'YTickLabel', flipud(groupImportanceTable.groupName(1:topGroupN)));
    xlabel('Mean decrease in held-out balanced accuracy')
    title('Top radial-SVM group permutation importance')
    grid on
