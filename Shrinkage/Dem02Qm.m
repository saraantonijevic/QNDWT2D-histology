...
%Add Gaussian noise with SNR = Var(signal)/Var(noise)
...
%Prepare a fast inverse operator;prefer transpose if numerically orthogonal
M = size(W,1);
if issparse(W)
    Iden = speye(M);
else
...
%arrange as N quaternion coefficients
% each row = [real, i, j, k]
Wcoef = reshape(wq, 4, []).';

%identify coefficient blocks; ordering is [A5, D5, D4, D3, D2, D1]
...
%exact inverse check with unthresholded coefficients
...
if mse_rec < targetMSE
    ...
    fprintf('Try widening the search box or switching filter family.\n');
end
...
plot(t, xnoisy, 'r')
grid on
title('Original and noisy Doppler signal')
